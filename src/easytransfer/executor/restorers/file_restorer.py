...
class FileRestorer:
    ...
    async def restore(
        ...
        Args:
            source_path: 包内的相对路径或原始绝对路径。
            target_path: 目标位置的绝对路径。

        Returns:
            备份文件/目录路径（如果有备份）；空字符串表示没有备份。
        ...
        if src is None:
            logger.warning("用户文件不在迁移包中: %s", source_path)
            raise RestoreError(
                f"用户文件不在迁移包中: {source_path}",
        ...
        logger.info("恢复文件: %s -> %s", src, dst)

        if self.dry_run:
            logger.info("[dry-run] 跳过实际文件恢复: %s", dst)
            return ""
    ...
    def _resolve_source(self, source_path: str) -> Path | None:
        ...
        # 2. 在 files/ 子目录下查找
        candidate = self.extract_dir / "files" / source_path
        if candidate.exists():
            return candidate
        ...
        if src_path.is_absolute():
            ...
            candidate = self.extract_dir / "files" / relative
            if candidate.exists():
                return candidate

            candidate = self.extract_dir / "files" / src_path.name
            if candidate.exists():
                return candidate
